...
import { getStudySessions } from 'apiSdk/study-sessions';
...
function DashboardPage() {
  ...
  const studySessionStat = useSWR<PaginatedInterface<StudySessionInterface>>(
    () => `/study-sessions`,
    () => getStudySessions({ limit: 1, offset: 0 }),
  );

  const textbookStat = useSWR<PaginatedInterface<TextbookInterface>>(
    () => `/textbooks`,
    () => getTextbooks({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: studySessionStat?.data?.totalCount,
      isLoading: studySessionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Textbooks',
      result: textbookStat?.data?.totalCount,
      isLoading: textbookStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Questions',
